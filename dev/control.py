...
class MPC(object):
    ...
    def _cost(self, u_k: np.ndarray, robot: Robot, target: tuple[int, int]):
        t: np.ndarray = np.array(target)
        crobot = deepcopy(robot)
        ...
        for i in range(self.horizon):
            crobot.set_robot_speeds(u_k[0, i], u_k[1, i])
            crobot.update(0.5)
            x, _ = crobot.state()
            z_k[:, i] = [x[0, 0], x[1, 0]]
    ...
    def update(self, robot: Robot, target: tuple) -> tuple[float, float]:
