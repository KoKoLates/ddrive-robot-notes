...
class Robot(object):
    def __init__(self, x: float, y: float) -> None:
        ...
        self.wheel_speeds: np.ndarray = np.reshape([0, 0], (2, 1))
    ...
    def update(self, dt) -> None:
        ...
        a: np.ndarray = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        ...
        vel: np.ndarray = np.array([[self.v[0, 0]], [self.v[2, 0]]])
    ...
    def _inverse(self) -> None:
        mat: np.ndarray = np.array(
            [[1 / self.r, 0, self.b / self.r], [1 / self.r, 0, -self.b / self.r]]
        )
